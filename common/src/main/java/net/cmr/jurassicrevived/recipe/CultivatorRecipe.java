...
//? if >1.20.1 {
/*public record CultivatorRecipe(NonNullList<Ingredient> inputs, ItemStack output, Map<ResourceLocation, Integer> weights) implements Recipe<CultivatorRecipeInput> {
 *///?} else {
public class CultivatorRecipe implements Recipe<CultivatorRecipeInput> {
...
//?}
	...
	//? if <=1.20.1 {
	public CultivatorRecipe(ResourceLocation id, NonNullList<Ingredient> inputs, ItemStack output, Map<ResourceLocation, Integer> weights) {
		this.id = id;
		this.inputs = inputs;
		this.output = output;
	...
	@Override
	public boolean matches(@NotNull CultivatorRecipeInput input, Level level) {
	...
	//? if >1.20.1 {
    /*@Override
    public @NotNull ItemStack assemble(CultivatorRecipeInput input, HolderLookup.Provider registries) {
    ...
    *///?} else {
	@Override
	public ItemStack assemble(CultivatorRecipeInput input, RegistryAccess registries) {
		return getWeightedResult(registries);
	}
	...
	public @NotNull RecipeSerializer<?> getSerializer() {
		return ModRecipes.CULTIVATOR_SERIALIZER.get();
	}

	@Override
	public @NotNull RecipeType<?> getType() {
		return ModRecipes.CULTIVATOR_RECIPE_TYPE.get();
	}
    ...
    *///?}
	...
	public static class Serializer implements RecipeSerializer<CultivatorRecipe> {
        ...
        public static final MapCodec<CultivatorRecipe> CODEC = RecordCodecBuilder.mapCodec(inst -> inst.group(
                ...
                }, DataResult::success).forGetter(CultivatorRecipe::inputs),
                ItemStack.CODEC.fieldOf("result").forGetter(CultivatorRecipe::output),
                WEIGHTS_CODEC.optionalFieldOf("fossil_weights", Map.of()).forGetter(CultivatorRecipe::weights)
        ).apply(inst, CultivatorRecipe::new));

            public static final StreamCodec<RegistryFriendlyByteBuf, CultivatorRecipe> STREAM_CODEC = StreamCodec.of(
                    (buf, recipe) -> {
                    ...
                    buf -> {
                        ...
                        return new CultivatorRecipe(inputs, output, weights);
                    }
            );

            @Override public MapCodec<CultivatorRecipe> codec() { return CODEC; }
            @Override public StreamCodec<RegistryFriendlyByteBuf, CultivatorRecipe> streamCodec() { return STREAM_CODEC; }
            *///?} else {
		@Override
		public CultivatorRecipe fromJson(ResourceLocation id, JsonObject json) {
			...
			return new CultivatorRecipe(id, inputs, output, weights);
		}
