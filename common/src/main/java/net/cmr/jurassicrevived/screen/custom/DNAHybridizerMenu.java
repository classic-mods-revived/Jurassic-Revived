...
public class DNAHybridizerMenu extends AbstractContainerMenu {
	...
	public DNAHybridizerMenu(int containerId, Inventory inventory, BlockEntity entity, ContainerData data) {
		...
		for (int i = 0; i < 8; i++) {
			int x = 8 + (i % 4) * 18;
			int y = 25 + (i / 4) * 18;
			this.addSlot(new Slot(blockEntity.itemHandler, i, x, y) {
				@Override
		...
		// Catalyst Slot (8)
		this.addSlot(new Slot(blockEntity.itemHandler, 8, 83, 35) {
			@Override
			public boolean mayPlace(ItemStack stack) {
		...
		// Output Slot (9)
		this.addSlot(new Slot(blockEntity.itemHandler, 9, 134, 35) {
			@Override
			public boolean mayPlace(ItemStack stack) {
				return false;
			}
