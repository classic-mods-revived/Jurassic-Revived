...
public class CultivatorMenu extends AbstractContainerMenu {
	...
	public CultivatorMenu(int containerId, Inventory inventory, BlockEntity entity, ContainerData data) {
		...
		// Water Input Slot (0)
		this.addSlot(new Slot(blockEntity.itemHandler, 0, 7, 61) {
			@Override
			public boolean mayPlace(ItemStack stack) {
				// Simplified validation for common: accepts bucket or water bucket
				return stack.is(Items.WATER_BUCKET);
			}
		});

		// Fossil Block Slot (1)
		this.addSlot(new Slot(blockEntity.itemHandler, 1, 56, 35) {
			@Override
			public boolean mayPlace(ItemStack stack) {
		...
		this.addSlot(new Slot(blockEntity.itemHandler, 3, 121, 35) {
			@Override
			public boolean mayPlace(ItemStack stack) {
				return false;
			}
		});
		this.addSlot(new Slot(blockEntity.itemHandler, 4, 139, 35) {
			@Override
			public boolean mayPlace(ItemStack stack) {
				return false;
			}
	...
	public ItemStack quickMoveStack(Player playerIn, int pIndex) {
		...
		if (pIndex < VANILLA_SLOT_COUNT) {
			// Player inventory -> machine inputs (slots 0 and 1)
