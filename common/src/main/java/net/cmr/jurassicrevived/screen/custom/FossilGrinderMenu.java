...
public class FossilGrinderMenu extends AbstractContainerMenu {
	...
	public FossilGrinderMenu(int containerId, Inventory inventory, BlockEntity entity, ContainerData data) {
		...
		// Fossil Input Slot (0)
		this.addSlot(new Slot(blockEntity.itemHandler, 0, 58, 35) {
			@Override
		...
		// Output Slots (1, 2, 3)
		this.addSlot(new Slot(blockEntity.itemHandler, 1, 103, 17) {
			@Override
			public boolean mayPlace(ItemStack stack) {
				return false;
			}
		...
		this.addSlot(new Slot(blockEntity.itemHandler, 3, 103, 53) {
			@Override
			public boolean mayPlace(ItemStack stack) {
				return false;
			}
	...
	public ItemStack quickMoveStack(Player playerIn, int pIndex) {
		...
		if (pIndex < VANILLA_SLOT_COUNT) {
			// Player inventory -> machine input (slot 0 only)
