...
import net.cmr.jurassicrevived.entity.client.MawsoniaVariant;
...
public class MawsoniaEntity extends DinoEntityBase implements GeoEntity {
	...
	private static final EntityDataAccessor<Integer> VARIANT =
		SynchedEntityData.defineId(MawsoniaEntity.class, EntityDataSerializers.INT);
	...
	private static final EntityDataAccessor<Float> DATA_ANIMAL_SCALE =
		SynchedEntityData.defineId(MawsoniaEntity.class, EntityDataSerializers.FLOAT);
	...
	public MawsoniaEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
	...
	//	return new ItemStack(ModItems.MAWSONIA_SPAWN_EGG.get());
	//}
	...
	public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
		AgeableMob child = ModEntities.MAWSONIA.get().create(pLevel);
		if (child instanceof MawsoniaEntity baby) {
			MawsoniaVariant randomVariant = Util.getRandom(MawsoniaVariant.values(), this.random);
	...
	public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
		controllers.add(new AnimationController<>(this, "SwimController", 5, state -> {
			...
			if (!this.isInWater()) {
				return state.setAndContinue(RawAnimation.begin().then("anim.mawsonia.flop", Animation.LoopType.LOOP));
			}

			// 2. Aquatic movement: Just swim
			return state.setAndContinue(RawAnimation.begin().then("anim.mawsonia.swim", Animation.LoopType.LOOP));
		}));

		controllers.add(new AnimationController<>(this, "attackController", 5, state -> PlayState.STOP)
			.triggerableAnim("attack", RawAnimation.begin().then("anim.mawsonia.attack", Animation.LoopType.PLAY_ONCE)));

		controllers.add(new AnimationController<>(this, "mouthController", 5, state -> PlayState.STOP)
			.triggerableAnim("mouth", RawAnimation.begin().then("anim.mawsonia.mouth", Animation.LoopType.PLAY_ONCE)));
	}
	...
	*//*?}*/
	...
	public MawsoniaVariant getVariant() {
		return MawsoniaVariant.byId(this.getTypeVariant() & 255);
	}

	private void setVariant(MawsoniaVariant variant) {
	...
	public boolean canMate(Animal other) {
		...
		if (!(other instanceof MawsoniaEntity that)) return false;
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pDataTag) {
		MawsoniaVariant variant = Util.getRandom(MawsoniaVariant.values(), this.random);
