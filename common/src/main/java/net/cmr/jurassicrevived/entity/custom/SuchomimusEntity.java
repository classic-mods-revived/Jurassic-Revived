...
import net.cmr.jurassicrevived.entity.client.SuchomimusVariant;
...
public class SuchomimusEntity extends DinoEntityBase implements GeoEntity {
    ...
    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(SuchomimusEntity.class, EntityDataSerializers.INT);
	...
	private static final EntityDataAccessor<Float> DATA_ANIMAL_SCALE =
		SynchedEntityData.defineId(SuchomimusEntity.class, EntityDataSerializers.FLOAT);
    ...
    public SuchomimusEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
	...
	public ItemStack getPickResult() {
		return new ItemStack(ModItems.SUCHOMIMUS_SPAWN_EGG.get());
	}
    ...
    public Block getEggBlock() {
        return ModBlocks.INCUBATED_SUCHOMIMUS_EGG.get();
    }
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.SUCHOMIMUS.get().create(pLevel);
        if (child instanceof SuchomimusEntity baby) {
            SuchomimusVariant randomVariant = Util.getRandom(SuchomimusVariant.values(), this.random);
    ...
    public boolean doHurtTarget(Entity target) {
        ...
        if (!level().isClientSide && hit && target instanceof LivingEntity) {
            if (this.level() instanceof ServerLevel serverLevel) {
                ...
                this.playSound(ModSounds.SUCHOMIMUS_ATTACK.get(), 1.0F, 1.0F);
            }
        }
        return hit;
    }
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        ...
        controllers.add(new AnimationController<>(this, "attackController", 5, state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.suchomimus.attack", Animation.LoopType.PLAY_ONCE)));
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.SUCHOMIMUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
	...
	protected void defineSynchedData(SynchedEntityData.Builder pBuilder) {
		super.defineSynchedData(pBuilder);    pBuilder.define(VARIANT, 0);    pBuilder.define(DATA_SYNCED_AGE, 0);    pBuilder.define(DATA_ANIMAL_SCALE, 1.0F);}
	*//*?}*/
...
public float getDinoScale() {
    ...
    public SuchomimusVariant getVariant() {
        return SuchomimusVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(SuchomimusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof SuchomimusEntity that)) return false;
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pDataTag) {
		SuchomimusVariant variant = Util.getRandom(SuchomimusVariant.values(), this.random);
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
		SuchomimusVariant variant = Util.getRandom(SuchomimusVariant.values(), this.random);    this.setVariant(variant);    this.setAnimalScale(Mth.nextFloat(this.random, MIN_ANIMAL_SCALE, MAX_ANIMAL_SCALE));    return super.finalizeSpawn(level, difficulty, spawnType, spawnGroupData);}
	*//*?}*/
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.SUCHOMIMUS_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.SUCHOMIMUS_DEATH.get();
    }

	@Override
	protected @Nullable SoundEvent getAmbientSound() {
		return ModSounds.SUCHOMIMUS_CALL.get();
	}
